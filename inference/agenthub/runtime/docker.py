...
from inference.repo_analysis.execution_log_parser import parse_log_fn, decolor_dict_keys
from inference.agenthub.runtime.base import (
...
from inference.agenthub import SUPPORTED_REPOS, SKIP_FILES, SKIP_FILES_NEW, CMD_TIMEOUT
import concurrent.futures

from inference.agenthub.trajectory.swebench_utils import (
    make_test_spec,
    swebench_parse,
    TestSpec,
)
from inference.agenthub.utils.utils import get_logger
from inference.commit_models.diff_classes import ParsedCommit
from inference.swesmith.utils import get_test_command
...
class DockerRuntime(ExecutionEnvironment):
    ...
    def __init__(
        ...
        **docker_kwargs,
        ...
        self.tool_repo_path = Path(tool_repo_path).expanduser().resolve() if tool_repo_path else None
        ...
        self.swefactory = "3a4b6b66" in self.docker_image or "swefactory" in self.docker_image
        # self.swefactory = True
        if self.swesmith:
        ...
        if self.swefactory:
            self.eval_script_content  = ds['eval_script']
            self.alt_path = "/opt"
        self.command = command
        self.repo_name = (
            self.ds["repo"] if self.swebench_verified or self.swesmith or self.swefactory else self.ds["repo_name"]
        )
        if not (self.swesmith or self.swefactory):
            self.commit_json = (
    ...
    def setup_env_swefactory(self):
        try:
            self.run("git config core.filemode false ")
            self.run(
                "printf 'testbed/\n.venv/\nvenv/\n__pycache__/\npytest_cache/\n*.egg-info\n' >> .git/info/exclude || true"
            )
            if not self.root_mode:
                self.run("pip install -e /opt/tools")
            else:
                ...
                self.copy_to_container(temp_file_path, f"{self.alt_path}/run_tests.sh")
                os.unlink(temp_file_path)  # Clean up the temporary file
                repo_path = "https://github.com/gnohgnailoug/swe_tool.git"
                self.run("pip install chardet")
                self.run(f"git clone {repo_path} /opt/tools")
                dest_path = shlex.quote("/opt/tools")
                self.run(f"pip install -e {dest_path}")
                # if self.tool_repo_path:
                #     self._prepare_tool_repo()
    ...
    def setup_env(self):
        ...
        elif self.swefactory:
            return self.setup_env_swefactory()
    ...
    def _prepare_tool_repo(self):
        tool_repo = self.tool_repo_path
        if not tool_repo:
            return
        if not tool_repo.is_dir():
            raise FileNotFoundError(f"tool_repo_path must be a directory, got: {tool_repo}")

        dest_path = "/opt/tools"
        self.copy_to_container(str(tool_repo), dest_path)

        install_cmd = f"pip install -e {shlex.quote(dest_path)}"
    ...
    def run(
        ...
        real_cmd = f"{exec_code} {args}".strip()
        ...
        if self.scaffold == "mini_swe_agent":
            if use_timeout_wrapper:
                ...
                exec_cmd = ["/bin/bash", "-lc", real_cmd]
        else:
            command = f"timeout {timeout} {real_cmd}".strip()
        ...
        try:
            ...
            if self.scaffold == "mini_swe_agent":
                ...
                env_kwargs["environment"] = {**quiet_env, **env_kwargs.get("environment", {})}
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
    ...
    def _calculate_reward_swefactory(self, get_test_output=False, timeout: int = 300) -> float:
        ...
        if output:
            ...
            print(f"[WARN] Empty test output content ")
    ...
    def _calculate_reward(self, get_test_output=False, timeout: int = 300) -> float:
        if self.swebench_verified:
            ...
            return self._calculate_reward_swefactory(get_test_output=get_test_output, timeout=timeout)
        else:
